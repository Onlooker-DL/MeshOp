function plotmeshrd3d(operatorName, operatorExperiment, testIds, methodNames)
...
%   rd3d_<method>_test<k>_id<id>.pdf/.png
    ...
    if nargin < 3 || isempty(testIds), testIds = 65; end
    ...
    outDir = fullfile(root,'figures','pltfig','reaction_diffusion','femmesh3d');
        ...
        for v = 1:numel(S.visualizationSamples)
            ...
            for m = 1:numel(methodNames)
                ...
                patch(ax,'Faces',bt,'Vertices',P, ...
                    'FaceColor',[0.82 0.90 0.98], ...
                    'EdgeColor',[0.25 0.35 0.5],'LineWidth',0.15);
                if ~isempty(st2)
                    ...
                    patch(ax,'Faces',st2,'Vertices',P3, ...
                        'FaceColor',[1.0 0.88 0.75], ...
                        'EdgeColor',[0.55 0.35 0.12],'LineWidth',0.15);
                end
                % unit-cube wireframe
                rd_draw_cube(ax);
                view(ax,3);
                ...
                title(ax,sprintf('%s, test %d (source %d)',mf,k,datasetIndex), ...
                    'FontSize',12);
                base = sprintf('rd3d_%s_test%03d_id%05d',mf,k,datasetIndex);
