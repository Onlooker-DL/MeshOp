...
def save_checkpoint(path: Path, model, optim, epoch: int) -> None:
    import torch
...
def load_checkpoint(path: Path, model, optim, device) -> int:
    import torch
...
def main() -> None:
    ...
    if args.resume and ckpt_path.exists():
        start_epoch = load_checkpoint(ckpt_path, model, optim, tcfg["device"]) + 1
    ...
    for ep in range(start_epoch, epochs + 1):
        ...
        if ep % save_every == 0 or ep == epochs:
            save_checkpoint(ckpt_path, model, optim, ep)
